...
Public Class Customer_SignUp
    ...
    Private Sub AddUser() ' method for inserting new user
        ...
        'SQL.AddParam("@phoneType1", phoneDrop1.DataSource) ' get drop down value
        'SQL.AddParam("@phoneNum1", num1.Text)
        'Extra numbers
        'SQL.AddParam("@phoneType2", phoneDrop2.DataSource)
        'SQL.AddParam("@phoneNum2", num2.Text)
        'SQL.AddParam("@phoneType3", phoneDrop3.DataSource)
        ...
        SQL.ExecuteQuery("INSERT INTO customer_data (first_name, last_name, city, state, " &
                         "zip_code, street, street_num, apartment_num, email, creation_date" &
                         " credit_card_num, type) VALUES (@first, @last, @city, @state, " &
                         "@zip, @street, @streetNum, @aptNum, @email, GETDATE(), @cardNum, " &
                         "@cardType);")
